...
# === CONFIGURATION ===
REMOVE_EXISTING_INSTALL=false  # Set to true to remove existing netcat and libmd packages before install
...
manage_package() {
    ...
    if eval "$check_cmd"; then
        ...
        if [ "$REMOVE_EXISTING_INSTALL" = true ]; then
            echo "REMOVE_EXISTING_INSTALL is set to true, starting removal of $display_name"
            if [ "$DRY_RUN" = true ]; then
...
# === LIBMD ===
pkg_name="libmd-1.1.0-x86_64-1cf.txz"
...
# === NETCAT ===
pkg_name="netcat-openbsd-1.229-x86_64-1cf.txz"
pkg_url="https://github.com/jcofer555/unraid_packages/raw/refs/heads/main/$pkg_name"
pkg_check="command -v nc >/dev/null 2>&1"
