...
REMOVE_EXISTING_P7ZIP=false  # Set to true to allow removal of old installation of p7zip
...
remove_p7zip() {
    if command -v 7z >/dev/null 2>&1; then
        ...
        if [ "$REMOVE_EXISTING_P7ZIP" = true ]; then
            echo "Removing existing p7zip installation..."
            removepkg p7zip >/dev/null 2>&1 && echo "Successfully removed p7zip" || echo "Failed to remove p7zip"
        else
            echo "REMOVE_EXISTING_P7ZIP is set to false, skipping removal"
        fi
    else
        echo "p7zip is not currently installed, nothing to remove"
    fi
}
