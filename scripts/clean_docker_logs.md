...
# =============================================================================
# clean_docker_logs
# Deterministic, atomic, auditable Docker container log truncation script.
# Skill: unified-jonathan-weighted-skill (Bash Module)
# =============================================================================
# DEVIATION NOTE: set -e is NOT used globally because find/truncate may
# return non-zero on permission-denied files which are logged but non-fatal.
...
# CONSTANTS
# =============================================================================
readonly SCRIPT_NAME="clean_docker_logs"
readonly WEBHOOK_URL="https://discord.com/api/webhooks/1389372076502028442/CN_Oo7nk1ZpKwT0zzNB7MB77mnxcUXBP3vLI0g3W6pJXbkVz4_E73mLVHElecjFvWSPIF"
readonly DEFAULT_LOG_DIR="/var/lib/docker/containers"
...
# =============================================================================
# CONFIGURABLE
# Override at runtime: LOG_DIR=/custom/path bash clean_docker_logs.sh
# =============================================================================
...
send_discord_error() {
    send_discord_message "CLEAN DOCKER LOGS ERROR" "$1" 15158332
}

send_discord_success() {
    send_discord_message "CLEAN DOCKER LOGS" "$1" 3066993
}
...
main() {
    ...
    # --- Root check ---
    # DEVIATION NOTE: root enforcement is a warning rather than a hard abort
    # because truncate on Docker log files may still succeed under some
    # non-root configurations (e.g. docker group membership). Uncomment
    # the exit 1 below to enforce strict root requirement.
    log_section "Checking permissions"
    if [[ $EUID -ne 0 ]]; then
        log_error "Script is NOT running as root — truncation may fail on protected log files"
        # exit 1
    else
    ...
    # --- Validate LOG_DIR ---
    log_section "Validating Docker log directory"
    if [ -z "$LOG_DIR" ]; then
        log_error "LOG_DIR is empty — this should not happen with default fallback set"
        exit 1
    fi

    if [ ! -d "$LOG_DIR" ]; then
        log_error "Docker log directory does not exist: $LOG_DIR"
        exit 1
    fi

    log_info "Docker log directory: $LOG_DIR"

    # --- Count logs before truncation for reporting ---
    local log_count
    log_count=$(find "$LOG_DIR" -type f -name "*.log" 2>/dev/null | wc -l)
    log_info "Found $log_count Docker log file(s) to truncate"

    # --- Truncate logs ---
    log_section "Truncating Docker log files"
    # DEVIATION NOTE: find -exec truncate runs per-file; individual failures
    # are non-fatal and logged via the exit code of find itself.
    if find "$LOG_DIR" -type f -name "*.log" -exec truncate -s 0 {} \; 2>&1 | tee -a "$LOG_FILE"; then
        log_info "Docker logs truncated successfully ($log_count files) in: $LOG_DIR"
    else
        log_error "One or more Docker log files could not be truncated in: $LOG_DIR"
    fi
