...
# =============================================================================
# reset_autobrr_logs
# Clears and recreates the autobrr download size and blocked log files.
# Skill: unified-jonathan-weighted-skill (Bash Module)
# =============================================================================
# DEVIATION NOTE: set -e is NOT used because rm -f on a non-existent file
# returns non-zero which is non-fatal here. set -uo pipefail is used instead.
set -uo pipefail

# =============================================================================
# CONSTANTS
# =============================================================================
readonly SCRIPT_NAME="reset_autobrr_logs"
...
readonly SIZE_LOG="/mnt/user/appdata/autobrr/scripts/download_size.log"
readonly BLOCKED_LOG="/mnt/user/appdata/autobrr/scripts/download_blocked.log"
readonly FILE_OWNER="jcofer555:users"
...
send_discord_error() {
    send_discord_message "RESET AUTOBRR LOGS ERROR" "$1" 15158332
}

send_discord_success() {
    send_discord_message "RESET AUTOBRR LOGS" "$1" 3066993
}
...
# =============================================================================
# RESET LOG FILE
# Removes and recreates a single log file, then sets ownership.
# Using > (truncate) instead of rm + touch — atomic single operation,
# no window where the file doesn't exist between delete and recreate.
# =============================================================================
reset_log_file() {
    local target="$1"
    ...
    if > "$target"; then
        log_info "Reset: $target"
    else
        log_error "Failed to reset: $target"
        return 1
    fi

    if chown "$FILE_OWNER" "$target"; then
        log_info "Ownership set: $target"
    else
        log_error "Failed to set ownership: $target"
    fi
}
...
main() {
    ...
    # --- Reset autobrr log files ---
    log_section "Resetting autobrr log files"
    reset_log_file "$SIZE_LOG"
    reset_log_file "$BLOCKED_LOG"
