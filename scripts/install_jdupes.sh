...
REMOVE_EXISTING_JDUPES=false  # Set to true to allow removal of old installation of jdupes

# DON'T CHANGE ANYTHING BELOW HERE

remove_jdupes() {
    if which jdupes >/dev/null 2>&1; then
        echo "jdupes is already installed."

        if [ "$REMOVE_EXISTING_JDUPES" = true ]; then
            echo "Removing existing jdupes installation..."
            removepkg jdupes >/dev/null 2>&1 && echo "Successfully removed jdupes" || echo "Failed to remove jdupes"
        else
            echo "REMOVE_EXISTING_JDUPES is set to false, skipping removal"
        fi
    else
        echo "jdupes is not installed, nothing to remove"
    fi
}
